...
export default function Page() {
  ...
  const [newMember, setNewMember] = useState({
    ...
    content: "",
    loading: false,
    date: new Date().toISOString().slice(0, 10), // current date and time in 'YYYY-MM-DD' format
  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await axios.post("/api/admin/reports/create", {
  ...
  const { mutate: editTeam, isPending } = useMutation({
    ...
    onError: () => {
      console.error("Failed to edit report");
    },
  });
  ...
  useEffect(() => {
    const nextTeam =
      data?.pages
        .flatMap((page) => (page?.reports as PostProps[]) ?? [])
        ?.filter(Boolean) ?? [];
                  ...
                  <div className="flex flex-col gap-4 w-full ">
                    ...
                    {/* <div className="flex flex-col">
                      ...
                      <input
                        type="time"
                        step="1"
                        value={newMember.time ? newMember.time : ""}
                        onChange={(e) =>
                          ...
                          <div className="flex flex-col gap-4 w-full ">
                            ...
                            <div className="flex flex-col">
                              ...
                              <input
                                type="date"
                                value={
                                  edit.date && edit.date.length === 8
                                    ? `${edit.date.substring(0, 4)}-${edit.date.substring(4, 6)}-${edit.date.substring(6, 8)}`
                                    : edit.date
                                }
                                onChange={(e) =>
                                  setEdit({
