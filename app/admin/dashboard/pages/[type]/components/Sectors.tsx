...
interface SectorsProps {
  type: string;
  rawSectors: Record<string, unknown>[];
}
...
export default function Sectors({ type, rawSectors }: SectorsProps) {
  ...
  useEffect(() => {
    ...
    const items = Array.isArray(rawSectors) ? rawSectors : [];
    setSectors(
      items.map((s) => ({
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await axios.post("/api/admin/pages/sectors/edit", {
        type,
        sectors,
      });
  ...
  const updateSector = (index: number, partial: Partial<Sector>) => {
    setSectors((prev) =>
  ...
  const removeSector = async (index: number) => {
    if (!confirm("Are you sure you want to remove this sector?")) return;
    const sector = sectors[index];
    if (sector.image) {
      try {
        await axios.post("/api/admin/pages/image/delete", {
          key: `pages/${type}/${sector.image}`,
        });
      } catch {
  ...
  const handleImageUpload = async (cardIndex: number, file: File) => {
    const oldImage = sectors[cardIndex].image;
    const uuid = uuidv4();
    ...
    try {
      ...
      updateSector(cardIndex, { image: uuid });
    } catch (error) {
  ...
  const removeImage = async (cardIndex: number) => {
    const imageKey = sectors[cardIndex].image;
    if (!imageKey) return;
    try {
      await axios.post("/api/admin/pages/image/delete", {
        ...
        >
          <div className="border-b border-gray-100 bg-gray-50 px-6 py-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-navy">
              Sector {idx + 1}
              {sector.title
                ? ` — ${sector.title.slice(0, 40)}${sector.title.length > 40 ? "..." : ""}`
                : ""}
            </h2>
            <button
              type="button"
          ...
          <div className="p-6 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                type="text"
                value={sector.title}
                onChange={(e) => updateSector(idx, { title: e.target.value })}
            ...
            <div className="flex flex-col gap-2">
              ...
              {sector.image ? (
                <div className="relative group w-48 h-32 rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                  <Image
                    src={`https://ukibc-storage.s3.ap-south-1.amazonaws.com/pages/${type}/${sector.image}`}
                    alt={`Sector ${idx + 1}`}
                    width={0}
                    height={0}
                    sizes="192px"
