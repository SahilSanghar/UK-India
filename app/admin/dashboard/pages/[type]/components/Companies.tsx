...
interface CompaniesProps {
  type: string;
  rawCompanies: Record<string, unknown>[];
}
...
export default function Companies({ type, rawCompanies }: CompaniesProps) {
  ...
  useEffect(() => {
    ...
    const items = Array.isArray(rawCompanies) ? rawCompanies : [];
    setCompanies(
      items.map((c) => ({
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await axios.post("/api/admin/pages/company/edit", {
        type,
        company: companies,
      });
  ...
  const updateCompany = (index: number, partial: Partial<Company>) => {
    setCompanies((prev) =>
  ...
  const handleImageUpload = async (cardIndex: number, file: File) => {
    const oldImage = companies[cardIndex].image;
    const uuid = uuidv4();
    ...
    try {
      ...
      updateCompany(cardIndex, { image: uuid });
    } catch (error) {
  ...
  const removeImage = async (cardIndex: number) => {
    const imageKey = companies[cardIndex].image;
    if (!imageKey) return;
    try {
      await axios.post("/api/admin/pages/image/delete", {
        ...
        >
          <div className="border-b border-gray-100 bg-gray-50 px-6 py-4 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-navy">
              Company {idx + 1}
              {company.title
                ? ` — ${company.title.slice(0, 40)}${company.title.length > 40 ? "..." : ""}`
                : ""}
            </h2>
            <button
              type="button"
          ...
          <div className="p-6 flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input
                type="text"
                value={company.title}
                onChange={(e) => updateCompany(idx, { title: e.target.value })}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input
                type="text"
                value={company.link}
                onChange={(e) => updateCompany(idx, { link: e.target.value })}
            ...
            <div className="flex flex-col gap-2">
              ...
              {company.image ? (
                <div className="relative group w-48 h-32 rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                  <Image
                    src={`https://ukibc-storage.s3.ap-south-1.amazonaws.com/pages/${type}/${company.image}`}
                    alt={`Company ${idx + 1}`}
                    width={0}
                    height={0}
                    sizes="192px"
