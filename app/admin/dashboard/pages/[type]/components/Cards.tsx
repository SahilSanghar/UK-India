...
export default function Cards({ type, rawCards }: CardsProps) {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const res = await axios.post("/api/admin/pages/cards/edit", {
        type,
        cards,
      });
      ...
      console.error("Failed to save cards section", error);
  ...
  const addCard = () => {
    setCards((prev) => [...prev, { title: "", des: "", link: "", image: "" }]);
  };
        ...
        >
          ...
          <div className="p-6 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <TextEditor
                content={card.des || ""}
                onChange={(des) => updateCard(idx, { des })}
              />
            </div>
