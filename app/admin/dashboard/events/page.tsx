...
export default function Page() {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useInfiniteQuery({
    queryKey: ['events'],
    queryFn: ({ pageParam }) => axios.get('/api/admin/events', {
      params: {
        limit: 12,
  ...
  return (
    ...
    <div className="w-full h-full flex flex-col overflow-x-hidden bg-white pb-10 pt-24">
      ...
      <p className='text-sm text-center flex items-center justify-center mb-10 bg-navy w-fit mx-auto rounded-full px-4 py-2 text-white'>Total Events: {data?.pages[0].count ?? 0}</p>
