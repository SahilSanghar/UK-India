...
export default function Page() {
  ...
  const handleCreate = async () => {
    ...
    try {
      const response = await axios.post("/api/admin/projects/create", {
  ...
  const { mutate: editTeam, isPending } = useMutation({
    ...
    onError: () => {
      console.error("Failed to edit case-study");
    },
  });
  ...
  useEffect(() => {
    const nextTeam =
      data?.pages
        .flatMap((page) => (page?.projects as PostProps[]) ?? [])
        ?.filter(Boolean) ?? [];
  ...
  const handleDelete = async (id: string, date: string) => {
    ...
    try {
      await axios.post("/api/admin/projects/delete", { id, date });
      queryClient.invalidateQueries({ queryKey: ["projects"] });
      setEdit({ ...edit, edit: false });
    } catch (error) {
    ...
    <>
      <div className="w-full h-full flex flex-col pb-10 pt-25 px-10 z-20">
        ...
        <p className="text-sm text-center flex items-center justify-center mb-5 bg-navy w-fit mx-auto rounded-full px-4 py-2 text-white ">
          Total case studies: {data?.pages[0].count ?? 0}
        </p>
            ...
            <>
              ...
              >
                ...
                <p className="text-sm text-navy font-bold">
                  {newMember.edit ? "Close" : "Add new case-study"}
                </p>
              </div>
