...
export default function layout({ children }: { children: React.ReactNode }) {
  ...
  const links = [
    ...
    {
      name: "Group Board",
      href: "/admin/dashboard/group-board",
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
      ...
      <div className="h-full flex-col w-full flex items-center">
        ...
        {links.map((link) => (
          <Link
            href={link.href}
            key={link.name}
            className="text-lg font-medium text-gray-500 hover:text-gray-900 hover:bg-gray-100 px-5 py-2 w-full text-left border-b border-gray-200 flex items-center gap-2"
          >
            {link.icon}
            {link.name}
          </Link>
