  ...
  try {
    ...
    await dynamoClient.send(
      new DeleteItemCommand({
        ...
        Key: { type: { S: "members" }, date: { S: date } },
    ...
    try {
      await s3Client.send(
        new DeleteObjectCommand({
          ...
          Key: `members/${trimmedId}`,
        }),
      );
    } catch (s3Error) {
      console.error(
        "Failed to delete image from ukibc-storage members:",
        s3Error,
      );
    ...
    try {
      await s3Client.send(
        new DeleteObjectCommand({
          ...
          Key: `members/${trimmedId}.webp`,
        }),
      );
    } catch (s3Error) {
      console.error(
        "Failed to delete optimized image from ukibc-optimized:",
        s3Error,
      );
      // Continue even if S3 delete fails - DynamoDB item is already deleted
    }
