...
export async function GET(req: NextRequest){
  try{
    ...
    const token = await new SignJWT({ username: user.Items[0].username, name: user.Items[0].name })
    ...
    return NextResponse.json({ message: "Unexpected error", error: e instanceof Error ? e.message : "Unknown error" }, { status: 500 });
  }
}